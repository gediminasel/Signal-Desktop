...
export function getNewLastSeenByUser(
  lastSeenMessageByUser:
    | Record<string, { receivedAt: number; id: string }>
    | undefined,
  ...
  const newLastSeenMessageByUser: Record<
    string,
    { receivedAt: number; id: string }
  > = {};
  ...
  for (const sourceConversationId of Object.keys(sendStateByConversationId)) {
    ...
    const old = lastSeenMessageByUser?.[sourceConversationId];
    if (!old || message.received_at > old.receivedAt) {
      newLastSeenMessageByUser[sourceConversationId] = {
        id: message.id,
        receivedAt: message.received_at,
      };
    }
  }
  return newLastSeenMessageByUser;
}
